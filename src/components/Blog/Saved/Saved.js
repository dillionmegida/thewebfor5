...
// when the id of a post is not provided, null is saved
// to the saved articles array in localstorage
// on the saved posts page, the number of saved items will read as array.length
// but the displayed posts are those with valid id.
// this anonymous function filters the null if it exists
;(() => {
  if (checkGlobal()) {
    ...
    render={(data) => {
      ...
      const savedPostsHeader = (
        <React.Fragment>
          ...
          <p className={Styles.HeaderExtraDetails}>
            {savedArticlesArray.length} post
            {savedArticlesArray.length === 1 ? "" : "s"} saved
          </p>
          <SaveMsg />
        </React.Fragment>
      )
      ...
      return (
        <Layout
          ...
          PageKeywords="saved"
          FirstSection={
            <div className={Styles.LeftContents}>{savedPostsHeader}</div>
          }
          SecondSection={
            <React.Fragment>
              <div className={Styles.PageHeader}>{savedPostsHeader}</div>
              {savedArticlesArray_Filtered.length !== 0 ? (
                savedArticlesArray_Filtered
                  .reverse()
                  .map((post) => (
                    <Post
                      key={post.id}
                      ...
                      extraInfo={`${formatBlogDate(post.frontmatter.date)} | ${
                        post.timeToRead
                      } min${post.timeToRead > 1 ? "s" : ""} read`}
                    />
                  ))
              ) : (
                <h2
                ...
                >
                  Oops : ( ... You have no saved articles
                </h2>
              )}
            </React.Fragment>
          }
