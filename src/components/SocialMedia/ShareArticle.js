...
const sharedText = (url, title, author) =>
  `${title} by @${author} on @${Brand.twitter} - ${
    Brand.domain
  }${url} %23${Brand.name.toLowerCase()}`
...
const nativeShare = (url, title, author) => {
  navigator
    ...
    .catch((err) => console.log("Couldn't share articel because ", err))
}
...
const OtherShare = (props) => {
  ...
  return (
    <>
      <ShareTwitter text={sharedText(url, title, author)} />
    </>
  )
}
        ...
        <div className={Styles.Methods}>
          {checkNativeShare() !== undefined ? (
            <button
              ...
              onClick={() => nativeShare(url, title, author)}
            >
              <Share /> Share
            </button>
          ) : (
            <OtherShare url={url} title={title} author={author} />
          )}
        </div>
      </div>
    </React.Fragment>
